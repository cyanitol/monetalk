...
| Cover traffic | 1 dummy stem per 120s per node (configurable up to 1/5 Hz) |

## Relay Peer Selection Algorithm

At each Dandelion epoch (120s), a node selects 4 relay peers uniformly at random from current outbound connections. Minimum 8 outbound connections required.
...
Every 120 seconds (+/- 15s jitter to desynchronize nodes):
...
- Each node sends 1 dummy stem transaction per 120 seconds (configurable: operators may increase rate up to 1 per 5 seconds for higher anonymity at increased bandwidth cost)
...
4. **Cover traffic**: 1 dummy stem per 120s (configurable up to 1/5 Hz)
...
Dandelion++ operates within epochs (120s), independent of chain cycles (11 days). At chain truncation boundary, the stem pool is flushed to fluff (all pending stems broadcast). A new Dandelion epoch begins with fresh relay selection.
