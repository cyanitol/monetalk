...
- Attacker creates N fake accounts, generates minimum activity (5 msgs sent, 5 received = 10 messages)
- Cost per fake account: PoW (10 min CPU) + initial grant (G MTK) + 10 * base_fee (minimum 100 MTK)
- Revenue per fake account: 0.10 * 10 * base_fee = 0.10 * 100 = 10 MTK
- Net loss per fake account: G + 90 MTK + PoW cost
- At minimum fee (10 MTK): cost = G + 100 MTK fees, bonus = 10 MTK, loss = G + 90 MTK
...
For a validator with 50,000 MTK stake and 6.75M MTK/cycle reward: $\mathbb{E}[\text{penalty}] \approx 12,500 + 10 \cdot 6,750 \approx 80,000$ MTK per cycle (present value). The cartel is profitable only if each member values censorship at >80,000 MTK per cycle.
